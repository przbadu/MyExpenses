...
export class DropboxDatabaseSync implements DatabaseSync {
    ...
    return NetInfo.fetch().then(({isConnected}) => {
      ...
      return LocalStorage.get(DROPBOX.ACCESS_TOKEN_STORAGE_KEY).then(
        accessToken => {
          ...
          let lastDropboxBackupTimestamp: Dayjs;
            ...
            if (dropboxMetadataResponse.status === 200) {
              ...
              return dropboxMetadataResponse
                ...
                .then(responseJson => {
                  ...
                  lastDropboxBackupTimestamp = dayjs(clientModifiedString);
                ...
                .then(lastLocalBackupTimestampString => {
                  ...
                  lastLocalBackupTimestamp = dayjs(
                    lastLocalBackupTimestampString,
                  );
  ...
  public hasLastUploadCompleted(): Promise<boolean> {
    return LocalStorage.get(DROPBOX.LAST_UPDATE_STATUS_KEY).then(
      lastUpdateStatus => {
  ...
  public hasSynced(): Promise<boolean> {
    return LocalStorage.get(DROPBOX.MOST_RECENT_BACKUP_TIMESTAMP_KEY).then(
      result => {
        if (result === null) {
          return false;
        } // Otherwise
